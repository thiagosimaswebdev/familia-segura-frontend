...
export default function AdminUsuarios() {
  ...
  async function aprovarRapido(id) {
    try {
      ...
      feedback(
        err.response?.data?.erro || "Erro ao aprovar",
        "erro"
      );
    }
  }
  ...
  async function excluirUsuario(id) {
    try {
      ...
      feedback(
        err.response?.data?.erro || "Erro ao excluir",
        "erro"
      );
    }
  }
  ...
  return (
    <div className="fade-in space-y-6 px-3 sm:px-0">

      {/* ===================================================== */}
      {/* CABEÇALHO */}
      {/* ===================================================== */}

      <div className="bg-gradient-to-r from-purple-700 to-purple-500 rounded-3xl p-5 sm:p-6 text-white">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5">
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            {[
              ...
              <div
                key={item.label}
                className={`${item.cor} backdrop-blur rounded-xl px-4 py-3 text-center min-w-[90px]`}
              >
      ...
      {pendentes.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-center gap-3 text-amber-700 text-sm">
          ...
          <span>
            ...
            usuário
            {pendentes.length > 1 ? "s" : ""} aguardando aprovação
          </span>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
        ...
        {/* ───────────────── DESKTOP ───────────────── */}
        {/* Mantém formato tabela no desktop */}

        <div className="hidden md:grid grid-cols-12 px-5 py-3 bg-slate-50 border-b border-slate-200 text-xs font-semibold text-slate-500 uppercase tracking-wide">
        ...
        {usuarios.length === 0 ? (
          ...
          usuarios.map((u, i) => (
            <div key={u.id}>
              ...
              <div
                className={`hidden md:grid grid-cols-12 px-5 py-4 border-b border-slate-100 text-sm items-center transition-colors hover:bg-slate-50/80 ${
                  u.status === "pendente"
              ...
              >
                ...
                <div className="col-span-2">
                  <span
                    className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full whitespace-nowrap ${STATUS_CONFIG[u.status]?.classe}`}
                  >
                    {STATUS_CONFIG[u.status]?.icone}
                ...
                <div className="col-span-2">
                  <span
                    className={`inline-flex items-center text-xs font-semibold px-2.5 py-1 rounded-full whitespace-nowrap ${ROLE_CONFIG[u.role]?.classe}`}
                  >
              ...
              <div
                className={`md:hidden p-4 border-b border-slate-100 space-y-4 ${
                  u.status === "pendente"
                    ? "border-l-4 border-l-amber-400"
                    : ""
                }`}
              >
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="font-semibold text-slate-800 break-words leading-tight">
                      {u.nome}
                    </p>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <span
                    className={`inline-flex items-center gap-1 text-xs font-semibold px-3 py-1 rounded-full ${STATUS_CONFIG[u.status]?.classe}`}
                  >
                    {STATUS_CONFIG[u.status]?.icone}
                    {STATUS_CONFIG[u.status]?.label}
                  </span>
                  ...
                  <span
                    className={`inline-flex items-center text-xs font-semibold px-3 py-1 rounded-full ${ROLE_CONFIG[u.role]?.classe}`}
                  >
                    {ROLE_CONFIG[u.role]?.label}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2 pt-1">
                  ...
                  {u.status === "pendente" &&
                    u.id !== usuario?.id && (
                      <button
                        ...
                        className="flex-1 h-10 bg-green-100 hover:bg-green-200 text-green-700 rounded-xl text-sm font-semibold transition-colors"
                      >
                        Aprovar
                      </button>
                    )}
                  ...
                  {u.id !== usuario?.id && (
                    <button
                      onClick={() => abrirEdicao(u)}
                      className="w-10 h-10 bg-blue-100 hover:bg-blue-200 text-blue-700 rounded-xl transition-colors"
                    >
                      ✎
                    </button>
                  )}
                  ...
                  {u.id !== usuario?.id && (
                    <button
                      ...
                      className="w-10 h-10 bg-red-100 hover:bg-red-200 text-red-600 rounded-xl transition-colors"
                    >
                      🗑
                    </button>
                  )}
