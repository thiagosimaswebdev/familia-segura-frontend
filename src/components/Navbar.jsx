...
export default function Navbar() {
  ...
  // Links privados — só para logados
  const linksPrivados = [
    { href: "/familias",  label: "Famílias"  },
    { href: "/dashboard", label: "Dashboard" },
  ];

  // Link admin — só para role admin
  const linkAdmin = { href: "/admin", label: "⚙️ Admin" };
    ...
    <nav className="bg-white border-b border-slate-200 sticky top-0 z-50 shadow-sm">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            {linksPublicos.map((link) => (
              <Link key={link.href} href={link.href} className={classeLink(link.href)}>
                {link.label}
              </Link>
            ))}

            {/* Links privados — só se logado */}
            {usuario && linksPrivados.map((link) => (
              <Link key={link.href} href={link.href} className={classeLink(link.href)}>
                {link.label}
              </Link>
            ))}
            ...
            {usuario?.role === "admin" && (
              <Link href={linkAdmin.href} className={`${classeLink(linkAdmin.href)} ${
                isAtivo(linkAdmin.href) ? "" : "text-purple-600 hover:bg-purple-50"
              }`}>
                {linkAdmin.label}
              </Link>
            )}
      ...
      {menuAberto && (
        <div className="md:hidden border-t border-slate-100 bg-white px-4 py-3 space-y-1">
          {linksPublicos.map((link) => (
            <Link key={link.href} href={link.href} className={classeLinkMobile(link.href)}>
              {link.label}
            </Link>
          ))}
          ...
          {usuario?.role === "admin" && (
            <Link href={linkAdmin.href} className={`${classeLinkMobile(linkAdmin.href)} text-purple-600`}>
              {linkAdmin.label}
            </Link>
          )}
